 ...
 * 2018-05-07: Added two fields for SWActor. They are <forcepoints> and <canUseForce>.
 *             Together with the getter and setter of each.(wen)
 * 2018-05-13: Added check statement for setting Lightsaber to WEAPON.
 *             Also added check statement for canceling Lightsaber's WEAPON Capability (wen)
 * 2018-05-27: Added getter for reference current world (wen)
 * 2018-05-27: Add hasInnerWorld indicator to show whether the SWActor has SWInnerWorld or not (wen)
 */
package starwars;
...
public abstract class SWActor extends Actor<SWActionInterface> implements SWEntityInterface {
	...
	/**If or not this <code>SWActor</code> can use the Force*/
	private boolean canUseForce;
	 ...
	 * @param   canUseForce show whether or not this SWActor can use the Force.
	 ...
	 */
	public SWActor(Team team, boolean canUseForce, int hitpoints, MessageRenderer m, SWWorldBase world) {
		super(m);
		actions = new HashSet<SWActionInterface>();
		this.team = team;
		this.canUseForce = canUseForce;
		this.forcepoints = 0;
	...
	/**
	 * getter for return the reference of the world.
	 * @return the current world this <code>SWActor</code> in
	 */
	public SWWorldBase getWorld() {
		return world;
	}
	
	/**
	 * Give a boolean value to show whether the SWActor can use Force
	 * @return True if the SWActor can use Force. False if cannot.
	 */
	public boolean getCanUseForce() {
		return canUseForce;
	}
	

	/**
	 * return the force points the SWActor has
	 * @return the SWActor's force points
	 */
	...
	/**
	 * add force points to the SWActor.
	 * @param forcepoints the force points to be added to the SWActor
	 */
	public void addForcepoints(int forcepoints) {
		if (canUseForce) {
			assert (forcepoints > 0): "Force Points cannot be negative!";
			this.forcepoints += forcepoints;
			if (this.getForcepoints() >= LightSaber.MARGIN_LEVEL && this.getItemCarried() instanceof LightSaber) {
				LightSaber myLightSaber = (LightSaber)this.getItemCarried();
				myLightSaber.setAsWeapon();
			}
		}
	}
	
	/**
	 * reduce force points for the SWActor.
	 * @param forcepoints the force points to be reduced from the SWActor
	 */
	...
	public void removeActionByExit() {
		if (actions != null) {
			for (Iterator<SWActionInterface> iterator = (Iterator<SWActionInterface>) actions.iterator(); iterator.hasNext();) {
			    SWActionInterface s =  iterator.next();
	...
	public void removeActionByEnter() {
		if (actions != null) {
			for (Iterator<SWActionInterface> iterator = (Iterator<SWActionInterface>) actions.iterator(); iterator.hasNext();) {
			    SWActionInterface s =  iterator.next();
			    if (s instanceof Enter) {
			        iterator.remove();
			    }       
			}
		}
